...
其中$1\leq x_1<\beta$，$0\leq x_j<\beta$，$j=2,3,\cdots,t$。$\beta^l$称为指数部分，指数$l$满足$L\leq l\leq U$，$L$与$U$分别为下界与上界；$x_1,x_2\cdots,x_t$称为位数。$fl(x)$称为一个\key{规格化浮点数}。
...
\entry 浮点数集中的数的个数：$N=2\cdot(\beta-1)\cdot\beta^t\cdot(U-L+1)+1$

\entry 浮点数$fl(x)$与对应真值$x$的误差：
\begin{itemize}\tl
    \item 绝对误差：$|x-fl(x)|\leq\dfrac12\beta^{-t}\times\beta^l=\dfrac12\beta^{l-t}$
    \item 相对误差：$\dfrac{|x-fl(x)|}{|x|}\leq\dfrac{\beta^{l-t}/2}{\beta^{l-1}}=\dfrac12\beta^{1-t}\ (|x|\geq0.1\times\beta^l)$
\end{itemize}
此类误差称为\key{舍入误差}。
...
\example 设$|x|\ll1$，则可以简化数值计算公式$\ln\dfrac{1-\sqrt{1-x^2}}{|x|}$为以下形式，以避免小数作分母：
...
\entry 针对一些模型，还存在着一类特定的问题，即\textbf{病态问题}。度量这类问题的性质，需要用到\key{条件数}。
\begin{itemize}\tl
...
\begin{gather}
\Delta y=\varphi(x_1,x_2,\cdots,x_n)-\varphi(\tilde{x}_1,\tilde{x}_2,\cdots,\tilde{x}_n)\approx\sum_{i=1}^n\frac{\partial\varphi(\tilde{x}_1,\tilde{x}_2,\cdots,\tilde{x}_n)}{\partial x_i}\cdot\frac{x_i}{y}\cdot\Delta x_i\\
\delta y=\frac{\Delta y}{y}\approx\sum_{i=1}^n\frac{\partial\varphi(\tilde{x}_1,\tilde{x}_2,\cdots,\tilde{x}_n)}{\partial x_i}\cdot\frac{x_i}{y}\cdot\frac{\Delta x_i}{x_i}=\sum_{i=1}^n\frac{\partial\varphi(\tilde{x}_1,\tilde{x}_2,\cdots,\tilde{x}_n)}{\partial x_i}\cdot\frac{x_i}{y}\cdot\delta x_i
\end{gather}
...
\item 总计运算量：$N=N_1+N_2=\frac{n^3}3+n^2-\frac n3=O(n^3)$
\end{itemize}
...
\section{Gauss消元法的改进}
\entry 列主元Gauss消元法：消去进行到第$k$步时如下所示：
...
\entry LU分解的算法实现：根据式(\ref{2-e1})可知，$\mathbf{A}$中的元素$\alpha_{ij}$满足
\begin{align*}
\alpha_{ij}&=(l_{i1}\ l_{i2}\ \cdots\ l_{i,i-1}\ 1\ 0\ \cdots\ 0)\cdot(\mu_{1j}\ \mu_{2j}\ \cdots\ \mu_{jj}\ 0\ \cdots\ 0)^{\mathrm{T}}\\
...
由此可以推得迭代算式为：
\begin{align}
&\mu_{1j}=\alpha_{1j}&j=1,2,\cdots,n\label{2-e2}\\
&l_{i1}=\alpha_{i1}/\mu{11}&i=2,3,\cdots,n\label{2-e3}\\
&\mu_{ij}=\alpha_{ij}-\sum\limits_{k=1}^{i-1}l_{ik}\mu_{kj}&j=i,i+1,\cdots,n\ i=2,3,\cdots,n\label{2-e4}\\
&l_{ki}=(\alpha_{ki}-\sum\limits_{t=1}^{i-1}l_{kt}\mu_{ti})/\mu_{ii}&k=i+1,\cdots,n\ i=2,3,\cdots,n\label{2-e5}
\end{align}
...
\entry 利用LU分解可解线性方程组$\mathbf{Ax}=\mathbf{b}$。
...
\begin{enumerate}\tl
    \item 非负性：$\forall \mathbf{x}\in\mathbb{R}^n,\|\mathbf{x}\|\geq0\text{且}\|\mathbf{x}\|=0\neq\mathbf{x=0}$
...
为分析舍入误差的相对水平，首先定义$\mathbf{e=\mathbf{x}^{\ast}-\tilde{\mathbf{x}}}$，分析其量级有
\[\tilde{\mathbf{x}}=\\\mathbf{A}^{-1}\mathbf{r}\sothat\|\mathbf{e}\|=\|\mathbf{A}^{-1}\mathbf{r}\|\leq\|\mathbf{A}^{-1}\|\cdot\|\mathbf{r}\|\]
...
\begin{itemize}\tl
    ...
    \footnote{可由其上的引理推出：通过反设$|\lambda|\geq1$退出$|\lambda\mathbf{I-B}|\neq0$，故$|\lambda|<1$，$\rho(\mathbf{B})<1$。}
    。
