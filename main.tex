...
\begin{gather}
g_{k+1}(x)=(x-b_x)g_k(x)-c_kg_{k-1}(x)\\b_k=\frac{\beta_k}{\gamma_k},
c_k=\frac{\gamma_k}{\gamma_{k-1}},\beta_k=(xg_k,g_k),\gamma_k=(g_k,g_k).\notag
\end{gather}
...
W=\diag(\omega_1,\omega_2,\cdots,\omega_m),\,y=
\begin{pmatrix}y_1\\y_2\\\cdots\\y_m\end{pmatrix},\,c=
\begin{pmatrix}c_0\\c_2\\\cdots\\c_n\end{pmatrix},\]
则原正规方程组化为
\begin{equation}
(G^TWG)c=(G^TW)y.
\end{equation}
...
此即最小二乘方程，具体形式为
\begin{equation}
\begin{pmatrix}\sum_{i=1}^m1&\sum_{i=1}^mx_i&\cdots&\sum_{i=1}^mx_i^n\\
\sum_{i=1}^mx_i&\sum_{i=1}^mx_i^2&\cdots&\sum_{i=1}^mx_i^{n+1}\\
\vdots&\vdots&\ddots&\vdots\\
\sum_{i=1}^mx_i^n&\sum_{i=1}^mx_i^{n+1}&\cdots&\sum_{i=1}^mx_i^{2n}\end{pmatrix}
\begin{pmatrix}c_0\\c_1\\\vdots\\c_n\end{pmatrix}=
\begin{pmatrix}\sum_{i=1}^my_i\\\sum_{i=1}^mx_iy_i\\\sum_{i=1}^mx_i^ny_i\end{pmatrix}
\end{equation}
